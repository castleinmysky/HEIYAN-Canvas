...
官方 API 和兼容中转可分别配置。Responses API 可按连接明确开启公开网页检索；默认关闭，Chat Completions 不启用。Agent 可发现并调用当前画布已有的节点、ComfyUI/H3 和任务能力。默认在生成前请求确认；主动开启「完全访问权限」后，可在当前画布自动执行，包括付费生成。权限按画布记忆，可随时切回。[1.3 更新说明](docs/releases-1.3.0.md)
