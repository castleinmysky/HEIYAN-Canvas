...
接自己的模型和 Codex，把素材与结果留在同一条创作流程里。官方 API 和兼容中转可分别配置；Agent 可创建、修改和连接节点，也可集中请求真实生成。付费生成永远需要你明确确认。
...
| Codex Agent | 连接个人 Codex，可选择当前账号实际可用的模型和思考程度，粘贴图片讨论需求、组织节点，并在确认后一次提交最多 4 个生成节点。 |
...
| Agent | 可分析本轮明确粘贴的 PNG、JPEG 或 WebP 图片；画布中的原始图片、视频和音频不会被自动发送，只会读取安全的节点、端口和连线信息。它也不会自动做完一部短片。 |
