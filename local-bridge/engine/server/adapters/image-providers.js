...
export function promptWithImageReferenceMap(prompt, referenceImages = []) {
...
export function geminiReferenceParts(prompt, referenceImages = []) {
...
export function createOpenAiImageAdapter(model) {
...
export function createGeminiImageAdapter(model) {
  const config = model.config || {};
  return {
    kind: 'image', configured: true,
    async run(job) {
