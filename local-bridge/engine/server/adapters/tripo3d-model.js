...
export function tripo3dRequestHash(request) {
  return crypto.createHash('sha256').update(canonicalJson({ endpoint: request.endpoint, body: request.body })).digest('hex');
}
...
export function createTripo3dModelAdapter(model) {
  ...
  const apiKey = String(config.apiKey || process.env[apiKeyEnvironment] || '').trim();
  ...
  return {
    ...
    async run(job) {
      ...
      const runStage = async ({ stageName, request, allowLegacyTaskId = false, progressFloor = 10, progressCeiling = 95 }) => {
        const requestHash = tripo3dRequestHash(request);
