...
export function validateAgentTool(tool, input) {
  ...
  if (tool === 'heiyan_read_canvas' || tool === 'heiyan_search_history') {
    ...
    if (input.nodeIds !== undefined && (!Array.isArray(input.nodeIds) || !input.nodeIds.every(id => bounded(id)))) throw Error('节点列表格式无效');
  ...
  if (tool === 'heiyan_read_generation' || tool === 'heiyan_review_result') {
    if (!exact(input, tool === 'heiyan_read_generation' ? ['jobs', 'waitSeconds'] : ['jobs', 'verdict', 'reason']) || !Array.isArray(input.jobs) || !input.jobs.length || tool === 'heiyan_review_result' && input.jobs.length !== 1 || !input.jobs.every(j => exact(j, ['nodeId', 'jobId']) && bounded(j.nodeId) && bounded(j.jobId))) throw Error('需要明确的节点与生成任务编号');
  ...
  if (tool === 'heiyan_request_generation') {
    if (!exact(input, ['summary', 'nodeIds']) || !bounded(input.summary, 1000) || !Array.isArray(input.nodeIds) || !input.nodeIds.length || !input.nodeIds.every(id => bounded(id))) throw Error('需要至少一个有效的生成节点');
    return { summary: input.summary, nodeIds: [...new Set(input.nodeIds)] };
  }
  if (tool !== 'heiyan_edit_canvas' || !exact(input, ['summary', 'operations']) || !bounded(input.summary, 1000) || !Array.isArray(input.operations) || !input.operations.length) throw Error('无效的画布操作');
...
export function sanitizeGenerationReport(value) {
  if (!Array.isArray(value)) throw Error('生成状态格式无效');
...
export function sanitizeAgentContext(value) {
  ...
  return { overview: object(value.overview) ? { totalNodes: Number(value.overview.totalNodes) || 0, totalEdges: Number(value.overview.totalEdges) || 0, matchedNodes: Number(value.overview.matchedNodes) || 0, nextOffset: Number.isInteger(value.overview.nextOffset) ? value.overview.nextOffset : null, promptOffset: Number(value.overview.promptOffset) || 0 } : undefined, revision: value.revision.slice(0, 100), viewport: bounds(value.viewport) && number(value.viewport.zoom) !== undefined ? { ...bounds(value.viewport), zoom: value.viewport.zoom } : undefined, nodes: value.nodes.slice(0, 200).map(n => ({ id: String(n.id).slice(0, 160), kind: String(n.kind).slice(0, 40), spatial: spatial(n.spatial), jobId: bounded(n.jobId) ? n.jobId : undefined, title: String(n.title || '').slice(0, 100), prompt: String(n.prompt || '').slice(0, 16000), promptLength: Number(n.promptLength) || String(n.prompt || '').length, promptTruncated: !!n.promptTruncated, settings: object(n.settings) ? { ratio: String(n.settings.ratio || '').slice(0, 40), resolution: String(n.settings.resolution || '').slice(0, 40), count: Number(n.settings.count) || 1, duration: Number(n.settings.duration) || 0 } : undefined, state: String(n.state || '').slice(0, 40), hasMedia: !!n.hasMedia, model: String(n.model || '').slice(0, 100), outputType: String(n.outputType || '').slice(0, 40), inputs: Array.isArray(n.inputs) ? n.inputs.filter(p => object(p) && bounded(p.id, 80)).slice(0, 24).map(p => ({ id: String(p.id).slice(0, 80), label: String(p.label || '').slice(0, 100), accepts: Array.isArray(p.accepts) ? p.accepts.filter(type => bounded(type, 40)).slice(0, 8) : [], multiple: !!p.multiple })) : [] })),
    edges: value.edges.slice(0, 500).map(e => ({ id: String(e.id || '').slice(0, 160), source: String(e.source).slice(0, 160), sourcePort: String(e.sourcePort || '').slice(0, 80), target: String(e.target).slice(0, 160), targetPort: String(e.targetPort || '').slice(0, 80), referenceToken: /^(图片|视频|音频|文本|模型|三视图集)[1-9]\d{0,5}$/.test(String(e.referenceToken || '')) ? e.referenceToken : '', type: String(e.type || '').slice(0, 40) })),
...
画布改动和真实生成都需要工具返回用户批准的实际结果，未返回成功不得宣称执行。节点创建、编辑、读取和生成没有人为设置的业务数量上限；大批量任务要覆盖用户要求的全部目标，不得自行缩减为前几个。执行层会按现有任务队列和可用并发逐项提交，仍须遵守模型真实输入输出规格、用户批准、成本、停止信号与服务稳定性约束。拒绝后不要绕过确认或重复申请同一操作。使用 availableModels 选择已配置的生成模型，通过 configure 设置其支持的比例、分辨率、数量和时长。没有可用模型或参数未开放时如实说明，不能伪造能力。
请如实区分规划、节点创建、排队、已生成资产和成片。当前没有剪辑合成工具，不能宣称已经完成一部短片。节点应保持命名清晰、结构可读；批量较大时用布局维持画布秩序。`;
...
const promptReferenceInstructions = '生成节点提示词引用已连入的素材时，使用最新 read_canvas 返回的 edges.referenceToken，写作 @图片1、@图片2 等，并明确各自承担的人物、服装、构图等用途。只使用 target 等于当前生成节点的连线；编号不是标题、节点 ID 或会话引用顺序，不得猜测或按数组位置重新编号。创建或替换连接后重新读取真实编号再写提示词；写入 @ 标记不会自动建立连接。无编号时不要伪造引用。';
export const contextInstructions = projectContextInstructions + resultInstructions + promptReferenceInstructions;
