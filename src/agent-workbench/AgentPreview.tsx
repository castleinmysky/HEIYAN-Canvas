...
export default function AgentPreview() {
  ...
  useEffect(() => {
    ...
    document.title = '黑岩画布 · Agent 功能演示';
  ...
  return <AgentWorkbench canvas={canvas} panel={panel} project={filmPlan ? '短片创作' : '角色场景探索'} theme={theme} onTheme={() => setTheme(value => value === 'night' ? 'day' : 'night')}
    ...
    {dialog === 'connection' && <WorkbenchDialog title="Codex 连接" onClose={() => setDialog(null)}><p>当前为功能演示，尚未连接 Codex，也不会发送模型请求。</p><p>连接 Codex 后，Agent 可通过受限接口操作当前画布；付费生成仍需确认。</p><button type="button" data-autofocus className="aw-primary" onClick={() => setDialog(null)}>继续体验界面</button></WorkbenchDialog>}
