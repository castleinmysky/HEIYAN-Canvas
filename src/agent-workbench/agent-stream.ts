...
export async function readAgentResponse(response: Response, protocol: 'responses' | 'chat', onText?: (text: string) => void): Promise<RawAnswer> {
  if (!response.headers.get('content-type')?.includes('text/event-stream')) {
    const result = parseAnswer(await response.json(), protocol); onText?.(result.text); return result;
  }
  ...
  const event = (block: string) => {
    ...
    if (data.error || ['error', 'response.failed', 'response.incomplete'].includes(data.type)) throw Error('模型流式响应失败；本次工具不会执行。');
    if (protocol === 'responses') {
