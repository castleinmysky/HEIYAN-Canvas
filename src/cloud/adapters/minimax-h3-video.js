...
export function bindMiniMaxProvider(fetch) {
...
function minimaxH3RequestBody({ model, prompt, inputs = [], referenceImages = [], options = {}, callbackUrl }) {
...
function createMiniMaxH3VideoAdapter(model) {
  ...
  return { kind: 'video', configured: true, async run(job) {
    ...
    let createPayload = { task_id: job.remoteTaskId };
    if (!createPayload.task_id) {
      ...
      if (!createResponse.ok || !createPayload.task_id) throw new Error(`MiniMax task submission returned HTTP ${createResponse.status}; no automatic retry was made.`);
    }
    ...
    while (true) {
      ...
      if (!response.ok) throw new Error(payload.base_resp?.status_msg || payload.error?.message || `MiniMax task query failed: HTTP ${response.status}`);
      ...
      if (['failed', 'cancelled'].includes(task.status)) throw new Error(task.error?.message || task.error || `MiniMax-H3 task ${task.status}`);
      attempts += 1;
      await job.onProgress?.(Math.min(92, 18 + Math.floor(Math.log2(attempts + 1) * 13)));
    }
  } };
}
return { createMiniMaxH3VideoAdapter, minimaxH3RequestBody };
}
