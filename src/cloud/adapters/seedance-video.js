...
// Existing HEIYAN wire contracts with explicit browser transport injection.
export function bindSeedanceProvider(fetch) {
function abortError() {
...
async function cancelRemoteTask(taskUrl, headers) {
  ...
  try { await fetch(taskUrl, { method: 'DELETE', headers, signal: controller.signal }); }
...
function seedanceRequestBody({ model, prompt, inputs = [], referenceImages = [], options = {}, callbackUrl }) {
  const content = [];
...
function createSeedanceVideoAdapter(model) {
  ...
  return {
    ...
    async run(job) {
      ...
      let createPayload = { id: job.remoteTaskId };
      if (!createPayload.id) {
        await job.onSubmissionStarted?.();
        const createResponse = await fetch(createUrl, { method: 'POST', headers, body: JSON.stringify(body), signal: job.signal, redirect: 'error' });
        createPayload = await createResponse.json().catch(() => ({}));
        if (!createResponse.ok || !createPayload.id) throw new Error(`Seedance task submission returned HTTP ${createResponse.status}; no automatic retry was made.`);
      }
      const taskUrl = `${createUrl}/${encodeURIComponent(createPayload.id)}`;
      const pollingDeadline = Date.now() + pollTimeoutMs;
      await job.onRemoteTask?.(createPayload.id);
      let attempts = 0;
      while (true) {
        ...
        const response = await fetch(taskUrl, { headers, signal: job.signal });
        const payload = await response.json().catch(() => ({}));
        if (!response.ok) throw new Error(payload.error?.message || payload.error || `Seedance 任务查询失败：HTTP ${response.status}`);
        ...
        if (['failed', 'cancelled', 'expired'].includes(payload.status)) throw new Error(payload.error?.message || `Seedance 任务${payload.status}`);
        attempts += 1;
...
return { createSeedanceVideoAdapter, seedanceRequestBody };
}
