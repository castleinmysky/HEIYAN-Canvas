...
export function ModelConnectionDialogFields({ value, apiKey, language, busy, error, onChange, onKeyChange }: {
  ...
  return <>
    ...
    {value.adapter === 'seedance-video' && language !== 'en' && <section className="model-provider-guide" aria-label="Seedance 自有账号接入"><strong>使用你自己的 Seedance 账户与额度</strong>{value.mode === 'official' ? <><p>① 在火山方舟开通账户可用的 Seedance 型号，按控制台要求完成认证和权限申请。</p><p>② 创建自己的 Ark API Key，填写获准使用的模型 ID 或接入点 ID（ep-…）。连接成功不等于所有型号都已获权。</p><p>③ 输入素材须满足所选型号的规则；涉及私域人像时，请使用当前账户已授权的素材资产，其他账户的 Asset ID 不能直接共享。</p><a href="https://console.volcengine.com/ark/region:ark+cn-beijing/apikey" target="_blank" rel="noreferrer">打开火山方舟 API Key 管理 ↗</a><a href="https://www.volcengine.com/docs/82379/1520758" target="_blank" rel="noreferrer">查看官方视频接口与素材规则 ↗</a></> : <p>填写你自己的中转密钥、模型 ID 和 HTTPS 地址。服务须同时兼容 Ark 创建任务、查询任务与取消任务协议；只有“返视频”的网页账号或不同格式的接口不能直接通用。</p>}<small>连接检查不会自动提交生成。费用由本连接所填密钥对应的服务商账户承担。</small></section>}
